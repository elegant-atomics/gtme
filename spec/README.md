...
| `schemas/field-registry.schema.json` | The `fields/*.json` type files themselves                | §4a, ADR-017/054 |
| `fields/<entity_type>.json` | A type: its `kind` (subject or signal), its ordered `identity` key tiers, and its canonical fields — names, tiers, normalization rules, value domains, and any `reference` that mints a relation | §4, §4a, ADR-017/054 |
